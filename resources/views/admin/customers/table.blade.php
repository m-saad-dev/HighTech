    ...
    <table class="table table-flush align-middle table-row-bordered table-row-solid gy-4 gs-9" style="max-width: inherit !important;">
        ...
        @foreach($staff as $staff)
            <tr>
                ...
                <td>
                    <a href="{{route('admin.staff.show', $staff->id)}}" class="badge badge-light-primary fs-7 fw-bold text-decoration-none">{{$staff->name}}</a>
                </td>
                <td>
                    <a href="javascript::void(0);" class="text-hover-primary text-gray-600">{{$staff->position}}</a>
                </td>
                <td>
                    <a href="javascript::void(0);" class="text-hover-primary text-gray-600">{{$staff->creator?->name}}</a>
                </td>
                <td>
                    <a href="javascript::void(0);" class="text-hover-primary text-gray-600">{{$staff->updater?->name}}</a>
                </td>
                @canany(['edit-staff', 'delete-staff'])
                        <td class="text-start">
                            <!--begin::Actions-->
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4 justify-content-center align-center" data-kt-menu="true">
                                ...
                                @can('edit-staff')
                                    <div class="menu-item px-3">
                                        <a href="{{route('admin.staff.edit', $staff->id)}}" class="menu-link px-3">@lang('common.edit')</a>
                                    </div>
                                @endcan
                                <!--end::Menu item-->
                                <!--begin::Menu item-->
                                @can('delete-staff')
                                        <form method="post" id="my_form" class="menu-item px-3" action="{{route('admin.staff.destroy', $staff->id)}}">
                                            @csrf @method('Delete')
