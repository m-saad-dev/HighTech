<div class="table-responsive">
    ...
    <table class="table table-flush align-middle table-row-bordered table-row-solid gy-4 gs-9" style="max-width: inherit !important;">
        ...
        <tr>
            ...
            <th class="min-w-100px">@lang('fields.title')</th>
            <th class="min-w-150px">@lang('fields.sub_title')</th>
        ...
        @foreach($articles as $article)
            <tr>
                ...
                <td>
                    <a href="{{route('admin.articles.show', $article->id)}}" class="badge badge-light-primary fs-7 fw-bold text-decoration-none">{{$article->title}}</a>
                </td>
                <td>
                    <a href="javascript::void(0);" class="text-hover-primary text-gray-600">{{$article->sub_title}}</a>
                </td>
                <td>
                    <a href="javascript::void(0);" class="text-hover-primary text-gray-600">{{$article->creator?->name}}</a>
                </td>
                <td>
                    <a href="javascript::void(0);" class="text-hover-primary text-gray-600">{{$article->updater?->name}}</a>
                </td>
                @canany(['edit-article', 'delete-article'])
                        <td class="text-start">
                            <!--begin::Actions-->
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4 justify-content-center align-center" data-kt-menu="true">
                                ...
                                @can('edit-article')
                                    <div class="menu-item px-3">
                                        <a href="{{route('admin.articles.edit', $article->id)}}" class="menu-link px-3">@lang('common.edit')</a>
                                    </div>
                                @endcan
                                <!--end::Menu item-->
                                <!--begin::Menu item-->
                                @can('delete-article')
                                        <form method="post" id="my_form" class="menu-item px-3" action="{{route('admin.articles.destroy', $article->id)}}">
                                            @csrf @method('Delete')
