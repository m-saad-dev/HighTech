...
@section('breadcrumb')
    @include('admin.layouts.breadcrumb_segmants', [
        'menu'      => [
            trans('menu.clients') => route('admin.clients.index'),
            trans('clients.editClient') => null,
        ],
    ])
@stop
@section('page_title')
    @lang('clients.editClient')
@stop
@section('actions')
...
@section('content')
    <div class="card mb-5 mb-xl-10">
        ...
        @endif
        <!--begin::Form-->
        <form class="form" action="{{route('admin.clients.update', ['client' => $client->id])}}" method="POST" enctype="multipart/form-data">
            @method('PATCH')
            @csrf
            @include('admin.clients.fields', $client)
            <!--begin::Actions-->
