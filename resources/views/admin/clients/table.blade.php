<div class="table-responsive">
    ...
    <table class="table table-flush align-middle table-row-bordered table-row-solid gy-4 gs-9" style="max-width: inherit !important;">
        ...
        <tr>
            ...
            <th class="min-w-150px">@lang('fields.position')</th>
        ...
        @foreach($clients as $client)
            <tr>
                ...
                <td>
                    <a href="{{route('admin.clients.show', $client->id)}}" class="badge badge-light-primary fs-7 fw-bold text-decoration-none">{{$client->name}}</a>
                </td>
                <td>
                    <a href="javascript::void(0);" class="text-hover-primary text-gray-600">{{$client->position}}</a>
                </td>
                <td>
                    <a href="javascript::void(0);" class="text-hover-primary text-gray-600">{{$client->creator?->name}}</a>
                </td>
                <td>
                    <a href="javascript::void(0);" class="text-hover-primary text-gray-600">{{$client->updater?->name}}</a>
                </td>
                @canany(['edit-client', 'delete-client'])
                        <td class="text-start">
                            <!--begin::Actions-->
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4 justify-content-center align-center" data-kt-menu="true">
                                ...
                                @can('edit-client')
                                    <div class="menu-item px-3">
                                        <a href="{{route('admin.clients.edit', $client->id)}}" class="menu-link px-3">@lang('common.edit')</a>
                                    </div>
                                @endcan
                                <!--end::Menu item-->
                                <!--begin::Menu item-->
                                @can('delete-client')
                                        <form method="post" id="my_form" class="menu-item px-3" action="{{route('admin.clients.destroy', $client->id)}}">
                                            @csrf @method('Delete')
